#!/usr/bin/env python3
"""Main execution script: handles user input and orchestrates the terrain generation."""
...
def prompt_float(msg: str, default: float | None = None) -> float:
    """Helper to safely prompt the user for a float value."""
...
def prompt_int(msg: str, default: int | None = None) -> int:
    """Helper to safely prompt the user for an integer value."""
    return int(prompt_float(msg, default))

def interactive_config() -> dict:
    """Walks the user through configuring the terrain area and coordinates via CLI prompts."""
    print("┌─────────────────────────────────────────┐")
    ...
    # Calculate grid boundaries early to present verification coordinates
    ...
    # Pause and force the user to verify alignment in-game before starting the heavy math
...
def run(cfg: dict):
    """Executes the pipeline: fetch -> resample -> map -> export."""
    ...
    # Calculate statistics for the final prompt
...
def main():
    # Setup optional config argument to bypass prompts
